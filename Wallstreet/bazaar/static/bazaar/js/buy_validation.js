document.getElementById("submitButton").onclick = function(){
    ...
    if(isNaN(bidPrice) || isNaN(quantity) || bidPrice<=0 || quantity<=0 || quantity>totalShares || bidPrice<0.9*currentPrice || bidPrice>1.1*currentPrice){
        document.getElementById("validation").style = "color:red"
        document.getElementById("cashValidation").style = "display:none"
    }
    else if(cash<=0 || cash<=(quantity*bidPrice*1.1)){
